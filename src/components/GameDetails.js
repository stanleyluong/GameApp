...
<div>
  {game.stores.map((store) => (
    <span key={store.id} className="store">{store.store.name}</span>
  ))}
</div>
...
<div>
  {game.developers.map((developer) => (
    <span key={developer.id} className="developer">{developer.name}</span>
  ))}
</div>
...
<div>
  {game.publishers.map((publisher) => (
    <span key={publisher.id} className="publisher">{publisher.name}</span>
  ))}
</div>
      ...
      <div className="game-screenshots">
        ...
        {game.screenshots?.map((screenshot, index) => (
          <img key={index} src={screenshot.image} alt="screenshot" className="game-screenshot" />
        ))}
      </div>

      {game.website && (
        <div className="game-website">
          <p>Official Website: <a href={game.website} target="_blank" rel="noopener noreferrer">{game.website}</a></p>
        </div>
      )}
    </div>
  );
